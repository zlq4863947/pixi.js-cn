 ...
 * 文本对象将创建一行或多行文本。
 *
 * 文字是使用 [Canvas API](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API) 创建的。
 *
 * 该类优于BitmapText的主要优点是，您可以很好地控制下一个的样式，可以在运行时进行更改。
 *
 * 主要缺点是每个文本都有其自己的纹理，这会占用更多的内存。
 * 文本更改时，必须重新生成此纹理并将其重新上传到GPU，这会占用时间。
 *
 * 要换行，可以在文本字符串中使用'\n'，也可以使用`style`对象，将 `wordWrap` 属性更改为true，并为 `wordWrapWidth` 属性赋予一个值。
 *
 * 可以直接从字符串和样式对象创建文本，然后在[这里](https://pixijs.io/pixi-text-style)生成样式对象
 *
 * ```js
...
{
    /**
     * @param {string} text - 您希望文本显示的字符串
     * @param {object|PIXI.TextStyle} [style] - 样式参数
     * @param {HTMLCanvasElement} [canvas] - 用于绘制文本的canvas元素
     */
